...
{	
	...
	if (id_news== null){
		$('<center><div class="form-group">'+
				'<div id="sub_div" class="col-lg-13">'+
		 			'<br><br><button type="submit" onclick="news_controller_create_news()" class="btn btn-primary">Save</button>'+
	...
	{	
		...
		$('<center><div class="form-group">'+
				'<div id="sub_div" class="col-lg-13">'+
		 			'<br><br><button type="submit" onclick="news_controller_update_news(' + id_news + ');" class="btn btn-primary">Save</button>'+
		 		'</div></div></center></form>').appendTo('#form_news');
	}
}
...
function news_view_show_news(news)
{
    ...
    $('<button onclick="news_view_create_form(' + news.id  +');" >Edit</button>&nbsp;&nbsp;&nbsp;').appendTo('#sub_div');
	$('&nbsp;<button onclick="news_controller_delete_news(' + news.id  +');" >Delete</button>').appendTo('#sub_div'); 
...
function show_all_news(){
   ...
   // $('<tr id="tr1" class="row_form"></tr>').appendTo('#table_items');
    ...
    for(var i in news){	
	    ...
	    $('<input type="button" value="Edit" onclick="news_controller_update_news('+news[i].id+')" />').appendTo('#td'+indice+'-2');
	    $('<input type="button" value="Delete" onclick="news_controller_delete_news('+news[i].id+')" />').appendTo('#td'+indice+'-2');
	    indice = indice + 1;
