...
function complaint_view_create_complaint(id_user){
	...
	jQuery('#container-complaint').html('<h3>Create Complaint Form</h3>'+
			...
			'<button  type="submit" onclick="complaint_controller_create_complaint('+id_user+')" class="btn btn-primary">Save</button>'+
			'</div>'+
			'</div>'+
			'</form>');
...
function complaint_view_list_complaints(){
		...
		jQuery('#container-section').append(
				...
				'<div class="col-xs-12 col-sm-12 col-md-2">'+
					'<ul id="selection-tags'+item.pk+'" class="meta-search">'+
						...
						'<a href="#" title="Affected user" onclick="user_view_show_profile_data( '+affected_user.id+' );"><li><i class="fa fa-user blue"></i><span class="blue">'+affected_user.username+'</span></li></a>'+
				...
				'<div class="col-xs-12 col-sm-12 col-md-7 excerpet">'+
	                ...
	                '<span class="plus"><a href="#" title="Send response" onclick="complaint_view_response_complaint('+item.pk+')"><i class="fa fa-comment"></i></a></span>'+
				'</div>'+
				'<span class="clearfix borda"></span>'+
			'</article>');
		
		if(typeof responsible_user !== 'undefined'){
			jQuery('#selection-tags'+item.pk).append('<a href="#" title="Responsible user" onclick="user_view_show_profile_data( '+responsible_user.id+' );"><li><i class="fa fa-user fa-fw red" ></i> <span class="red">'+responsible_user.username+'</span></li></a>');
		 }
...
function complaint_view_response_complaint(id_complaint){
	...
	jQuery('#container-complaint').append(
			...
			'<div class="col-xs-12 col-sm-12 col-md-2">'+
				'<ul id="selection-tags'+id_complaint+'" class="meta-search">'+
					'<li><i class="glyphicon glyphicon-calendar blue"></i> <span>FECHA</span></li>'+
					'<a href="#" title="Affected user" onclick="user_view_show_profile_data( '+ affected_user.username +' );"><li><i class="fa fa-user blue"></i><span class="blue">'+complaint.affected_user.username+'</span></li></a>'+
					'<a href="# title="Related Swap"><li><i class="fa fa-shopping-cart orange"></i><span class="orange"> SWAP </span></li></a>'+
					'</ul>'+
			'</div>'+
			'<div class="col-xs-12 col-sm-12 col-md-7 excerpet">'+
	...
	jQuery('#container-complaint').append('<h3>Response the complaint</h3>'+
			...
			'<button  type="submit" onclick="complaint_controller_send_response('+affected_user.id+','+id_complaint+')" class="btn btn-primary">Send response</button>'+
			'</div>'+
			'</div>'+
			'</form>');
