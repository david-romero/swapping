...
function shopping_cart_view_show()
{
	...
	else
	{
		...
		jQuery('#tablas').append('<table  class="table table_header">'+
				'<thead>'+
				'<tr>'+
					'<th id="service_description" colspan="2" class="description" >'+
						'Description:'+
					'</th>'+
					'<th id="created" class="date">'+
						'Created'+
					'</th>'+
					'<th id="list_itemcategory" class="category">'+
						'Category'+
					'</th>'+
					'<th id="quantity" class="quantity">'+
						'Quantity'+
					'</th>'+
					'<th id="service_price" class="price">'+
						'Price'+
					'</th>'+
					'<th class="subtotal">'+
						'Subtotal'+
			...
			jQuery.each(shopping_cart_items[i].fields, function(fieldname, value) 
					{
			            ...
			            if (fieldname == 'user'){
			            	...
			            	username = values[1];
			            	id = values[0];
			            	jQuery('#user' + i).append('<a onclick="user_view_show_profile_data(' + id + ');refreshLanguage();" href="#">'+ username + '</a>');
			            }
			            if (fieldname == 'price')
			            {
		...
		jQuery('#shopping_cart_items').append('<div class="table-responsive order_blocks clearfix">'+
				'<div class="row">'+
					'<div class="span6 shipping_block clearfix">'+
					 '<div class="delivery_block pull-left">'+	
						'<h4 id="delivery_method">Delivery Method</h4>'+
				...
				'<div class="span6 total_block">'+
					'<h4 id="summary_block">Summary Block</h4>'+
					'<table class="table" id="basketTotalsList">'+
						'<tbody>'+
							'<tr>'+
								'<td id="goods_subtotal" class="total_position">Goods Subtotal</td>'+
								'<td class="basket_subtotal">'+
									'<span class="amount">' + goods_total + '</span><span class="currency">&euro;</span>'+
								'</td>'+
							'</tr>'+
							'<tr>'+
								'<td class="total_position"><div id="taxes_inf">Taxes</div> (<span class="taxes_percent">2</span>%)</td>'+
								'<td class="basket_taxes">'+
									'<span class="amount">' + taxes + '</span><span class="currency">&euro;</span>'+
								'</td>'+
							'</tr>'+
							'<tr>'+
								'<td id="delivery_cost" class="total_position">Delivery cost</td>'+
							...
							'<tr class="total">'+
								'<td class="total_position">'+
									'<h3 id="order_total">Order Total</h3>'+
								'</td>'+
		...
		jQuery('#shopping_cart_items').append('<div class="navbar">'+
				...
				'<ul class="nav custom_nav">'+
					'<li class="pull-left">'+
						'<div>'+
							'<a id="continue_shop" href="/home" class="btn btn-info">'+
					...
					'</li>'+
					'<li class="pull-right">'+
						'<button id="continue_checkout" onclick="swapping_with_money_controller_payment();refreshLanguage();" class="btn btn-success continue_to_checkout" type="submit" value="Checkout" name="cont_to_checkout2">'+
					...
					'<li class="pull-right">'+
						'<div>'+
							'<button id="remove_all_button" onclick="shopping_cart_controller_remove_all();refreshLanguage();" class="btn btn-danger remove_all" type="submit" name="remove_all2" value="remove_all" rel="tooltip" data-placement="top" data-original-title="Be careful! All products will be entirely removed from the shopping cart!">'+
	...
	jQuery('#tablas').append('<table  class="table" >'+
			'<tbody>'+
				...
				'<tr class="warning additional">'+
					 '<td colspan="6" id="user' + index + '"><b id="service_owner">Owner:</b> &nbsp;'+
					 '</td>'+
				'</tr>'+
				'<tr class="">'+
...
function shopping_cart_view_check_exists_items()
{
	...
	if (shopping_cart_items != undefined)
		...
		{
			jQuery('#shopping_cart_container a').html('<i class="fa fa-shopping-cart"></i>&nbsp;&nbsp;<span>' + shopping_cart_items.length + '</span>');
		}
}
