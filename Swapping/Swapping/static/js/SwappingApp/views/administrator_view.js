function administrator_view_show_form(id_admin)
{
	...
	jQuery('#container-admin').html('<h3>Administrator Register Form</h3>'+
			...
			'<button  type="submit" '+
			'onclick="' + (id_admin==undefined ? 'administrator_controller_add_admin();refreshLanguage();"' : 'administrator_controller_update_admin(' + id_admin + ');refreshLanguage();"')+
...
function administrator_view_show_profile_data(admin)
{
	...
	jQuery('<a class="btn btn-default" style="padding-right: 18px;" onclick="administrator_view_show_form(' + admin.id  +');refreshLanguage();" href="#"><i class="fa fa-pencil fa-fw"></i> Edit</a><br style="clear:both;"/>').appendTo('#sub_div');
...
function administrator_view_show_ban_user()
{
	administrator_controller_get_all_users();
}


function administrator_view_show_users(users)
{
	body_clear();
	jQuery('<table id="table_ban_user" class="table table-hover table-striped table-bordered">'+
			'<thead><th>Username</th><th>First Name</th><th>Last Name</th><th>Last Login</th><th>Photo</th><th>Ban</th></thead>'+
			'<tbody></tbody></table>').appendTo('#content-body');
	
	jQuery.each(users, 
			function (index, user)
			{
				var photo = user_controller_get_photo(user.id, true);
 				jQuery('#table_ban_user tbody').append('<tr>'+
							'<td>'+
								user.username +
							'</td>'+
							'<td>'+
							...
							'</td>'+
							'<td>'+
								'<img style="height: 5em;" class="img-responsive img-circle" src="' + photo.url + '" />' +
							'</td>'+
							'<td>'+
								(user.is_active?'<i   ' + (user.is_active==true?'style="cursor:pointer;" onclick="administrator_controller_ban_user(' + user.id + ');refreshLanguage();"':' style="display:none;"') +'class="fa fa-ban"></i>':
									'<i   ' + (user.is_active==false?'style="cursor:pointer;" onclick="administrator_controller_unlock_user(' + user.id + ');refreshLanguage();"':' style="display:none;"') +'class="fa fa-unlock-alt"></i>') +
